...
def index():
    ...
    stock_data_map["current_lang"] = lang
    t = translations[lang]
    
    # 提供默认值
    default_code = stock_data_map["code"] or "cn_600919"
    default_start = stock_data_map["start_date"] or "20250101"
    default_end = stock_data_map["end_date"] or "20260203"
...
def fetch_data():
    ...
    # 更新全局变量
    stock_data_map["code"] = stock_code
    stock_data_map["start_date"] = start_date
    stock_data_map["end_date"] = end_date
    ...
    try:
        ...
        if formatted_data:
            stock_data_map["data"] = formatted_data
            ...
            if len(data_json) > 0 and 'stat' in data_json[0]:
                ...
                stock_data_map["cumulative_data"] = {
            ...
            return jsonify({
                'success': True, 
                'data': formatted_data,
                'cumulative_data': stock_data_map["cumulative_data"],
...
def get_chart_data():
    ...
    for date, values in stock_data_map["data"].items():
        dates.append(date)
...
def export_csv():
    ...
    response.headers['Content-Disposition'] = f'attachment; filename=stock_data_{stock_data_map["code"]}.csv'
